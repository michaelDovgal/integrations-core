...
def _define_validator_functions(model_id, validator_data):
    ...
    model_file_lines.append("        if info.field_name in info.context['configured_fields']:")
    model_file_lines.append('            return value')
    model_file_lines.append('')
    model_file_lines.append('        field = cls.model_fields[info.field_name]')
    model_file_lines.append(f"        return getattr(defaults, f'{model_id}_{{info.field_name}}')(field, value)")

    model_file_lines.append('')
    model_file_lines.append("    @field_validator('*')")
    model_file_lines.append('    def _run_validations(cls, value, info):')
    model_file_lines.append("        if info.field_name not in info.context['configured_fields']:")
    model_file_lines.append('            return value')
    model_file_lines.append('')
    model_file_lines.append('        field = cls.model_fields[info.field_name]')
    model_file_lines.append(
